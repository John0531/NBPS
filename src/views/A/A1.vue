      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            ...
            <h6>供<span class="text-primary">帳務科經辦</span>上傳批次交易資料檔，請於營業日 <span class="text-primary">09:00-18:00</span>
              期間執行，批次交易資料檔上傳且格式檢核成功後，按下確認並送出，系統隨開始發動交易。</h6>
            <h6 class="text-danger fw-bold">*檔名為: 商店代碼後九碼 + "." + YYYYMMDD(當天日期) + "." +
              A(第一次上傳為A，第二次為B，以此類推)+"."txt(大、小寫皆可)。例如：XXXXXXXXX.20230317.A.txt(TXT)。</h6>
            <h6 class="text-danger fw-bold">*上傳檔案均需依約定之密碼規則(約定之加解密密碼+兩碼當月月份)，以ZIP加密，若檔名或解密錯誤或總筆數及總金額不對，此檔直接剔退。</h6>
          </div>
          <div class="card-body">
            <Form ref="form" v-slot="{ errors }" @submit="uploadFile">
              <div class="row py-3">
                <div class="col-xxl-6 d-flex mb-4 align-items-center">
                  <h5 class="text-nowrap me-3" style="padding-top:0.375rem;">特店代碼:</h5>
                  <Field style="width:300px;" id="batchStoreId" class="form-select" as="select" rules="required"
                    :class="{ 'is-invalid': errors['特店代碼'] }" name="特店代碼" v-model="uploadPost.storeId">
                    <option value="" selected>請選擇</option>
                    <option v-for="item in defaultData" :key="item.id" :value="item.storeId">{{ item.storeId }}
                      ({{ item.storeName }})</option>
                  </Field>
                  <ErrorMessage name="特店代碼" class="invalid-feedback ms-2" />
                </div>
                <div class="col-xxl-6"></div>
                <div class="col-xxl-6 d-flex mb-4 align-items-center">
                  <h5 class="text-nowrap me-3" style="padding-top:0.375rem;">批次交易檔:</h5>
                  <Field id="batchFile" style="width:400px;" ref="upload" type="file" class="form-control" placeholder=""
                    rules="required" name="批次交易檔" :class="{ 'is-invalid': errors['批次交易檔'] }" @change="getFile($event)" />
                  <ErrorMessage name="批次交易檔" class="invalid-feedback ms-2" />
                </div>
                <div class="col-xxl-6"></div>
                <div class="col-xxl-3 d-flex mb-4 align-items-center">
                  <h5 class="text-nowrap me-3" style="padding-top:0.375rem;">總筆數:</h5>
                  <Field name="總筆數" type="number" class="form-control" rules="required"
                    :class="{ 'is-invalid': errors['總筆數'] }" id="totalCount" v-model.number="uploadPost.totalCount" />
                  <ErrorMessage name="總筆數" class="invalid-feedback ms-2" />
                </div>
                <div class="col-xxl-5 d-flex mb-4 align-items-center">
                  <h5 class="text-nowrap me-3" style="padding-top:0.375rem;">總金額(授權金額-退貨金額):</h5>
                  <Field name="總金額" type="number" class="form-control" rules="required"
                    :class="{ 'is-invalid': errors['總金額'] }" id="totalAmt" v-model.number="uploadPost.totalAmt" />
                  <ErrorMessage name="總金額" class="invalid-feedback ms-2" />
                </div>
              ...
              <button type="submit" class="btn btn-primary me-3 px-4"
                :disabled="!$store.state.pageBtnPermission.includes('insert')">上傳</button>
              <button class="btn btn-warning me-3 px-4" @click.prevent="downloadExcel"
                :disabled="!$store.state.pageBtnPermission.includes('download')">下載範例EXCEL</button>
              <button class="btn btn-success me-3 px-4" @click.prevent="downloadFormat"
                :disabled="!$store.state.pageBtnPermission.includes('download')">下載批次交易檔規格</button>
              <button class="btn btn-info me-3 px-4" @click.prevent="convertFile"
                :disabled="!$store.state.pageBtnPermission.includes('execute')">UB BPS轉檔程式</button>
            </Form>
          </div>
        </div>
          ...
          <template #default>
            ...
            <tbody>
              <tr v-for="item in gridData" :key="item.batchId">
                ...
                <td>
                  <button v-if="item.batchStatus === 'VALIDATE_FAIL'" @click="getError(item)"
                    class="btn btn-danger me-2 btn-sm">檢視錯誤</button>
                  <button v-if="item.batchStatus === 'VALIDATE_SUCCESS'" @click="getDetail(item)"
                    class="btn btn-success me-2 btn-sm">檢視明細</button>
                  <button v-if="item.batchStatus === 'VALIDATE_SUCCESS'" @click="confirmBatch(item)"
                    class="btn btn-primary btn-sm"
                    :disabled="!$store.state.pageBtnPermission.includes('execute')">確認送出授權</button>
                </td>
              </tr>
            </tbody>
          </template>
            ...
            <div class="row justify-content-center">
              <div class="col-12">
                <div class="card">
                  ...
                  <div class="card-body">
                    <Form ref="excelForm" v-slot="{ errors }" @submit="uploadExcelFile">
                      <div class="row py-3">
                        <div class="col-xxl-6 d-flex mb-4 align-items-center">
                          <h5 class="text-nowrap me-3" style="padding-top:0.375rem;">上傳檔案:</h5>
                          <Field id="excelFile" style="width:400px;" ref="upload" type="file" class="form-control"
                            placeholder="" rules="required" name="上傳檔案" :class="{ 'is-invalid': errors['上傳檔案'] }"
                            @change="getFileConv($event)" />
                          <ErrorMessage name="上傳檔案" class="invalid-feedback ms-2" />
                        </div>
                        <div class="col-xxl-6"></div>
                      </div>
                      <button type="submit" :disabled="!$store.state.pageBtnPermission.includes('execute')"
                        class="btn btn-success me-3 px-4">轉檔</button>
                    </Form>
                  </div>
                </div>
                <br>
                <div class="card">
                  ...
                  <div class="card-body">
                    <Form ref="txtForm" v-slot="{ errors }" @submit="uploadTxtFile">
                      <div class="row py-3">
                        <div class="col-xxl-6 d-flex mb-4 align-items-center">
                          <h5 class="text-nowrap me-3" style="padding-top:0.375rem;">上傳檔案:</h5>
                          <Field id="txtFile" style="width:400px;" ref="upload" type="file" class="form-control"
                            placeholder="" rules="required" name="上傳檔案" :class="{ 'is-invalid': errors['上傳檔案'] }"
                            @change="getFileConv($event)" />
                          <ErrorMessage name="上傳檔案" class="invalid-feedback ms-2" />
                        </div>
